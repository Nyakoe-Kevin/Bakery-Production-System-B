...
const openCompletionPanel = (batch) => {
  ...
  actualInput.value = batch.actual_quantity ?? 0
  wasteInput.value = batch.wastage_quantity ?? 0
...
const confirmBatchCompletion = () => {
  ...
  selectedBatch.value.actual_quantity = actualInput.value
  selectedBatch.value.wastage_quantity = wasteInput.value
  selectedBatch.value.status = 'done'
  selectedBatch.value.completed_at = new Date().toISOString()
  ...
  <div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      <BatchCard
        ...
        @request-completion="openCompletionPanel"
        @mark-failed="markFailed"
      />
    </div>

    <div v-if="showCompletionPanel" class="mt-6 bg-white p-6 rounded-xl shadow-lg">
      <h2 class="text-xl font-semibold mb-4">
        Complete Batch: {{ selectedBatch?.product_name }}
      </h2>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <label class="block text-sm text-gray-600 mb-1">Actual Quantity</label>
          <input
            type="number"
            v-model.number="actualInput"
            class="w-full border rounded-lg p-3"
          />
        </div>

        <div>
          <label class="block text-sm text-gray-600 mb-1">Wastage</label>
          <input
            type="number"
            v-model.number="wasteInput"
            class="w-full border rounded-lg p-3"
          />
        </div>
      </div>
      ...
      <div class="flex gap-3">
        <button
          @click="confirmBatchCompletion"
          class="bg-green-600 text-white px-5 py-3 rounded-lg hover:bg-green-700"
        >
          Confirm
        </button>
        <button
          @click="showCompletionPanel = false"
          class="bg-gray-200 text-gray-800 px-5 py-3 rounded-lg"
        >
          Cancel
        </button>
      </div>
